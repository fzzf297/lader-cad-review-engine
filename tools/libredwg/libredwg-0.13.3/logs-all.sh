...
srcdir="."
abs_builddir="/Users/fzzf/work/code/cad/tools/libredwg/libredwg-0.13.3"
...
for d in "$srcdir"/test/test-data/2*/*.dwg "$srcdir"/test/test-data/r*/*.dwg; do
    ...
    echo "$d" || exit
     ./libtool --mode=execute programs/dwgread -v4 "$d" 2>"$log"
...
for d in "$srcdir"/test/test-big/2*/*.dwg; do
    ...
    log="$(basename "$d" .dwg)_$dir.log"
    echo "$d" || exit
     ./libtool --mode=execute programs/dwgread -v3 "$d" 2>"$log"
    #(sleep 10s; kill %1 2>/dev/null) &
done
    ...
    for d in r* 20*; do
        ...
        for f in */*.dwg; do
            ...
            log="$(basename "$f" .dwg)_${dir}_${d}.log"
             "$abs_builddir/libtool" --mode=execute "$abs_builddir/programs/dwgread" -v4 "$f" 2>"$abs_builddir/$log"
        done
        cd .. || exit
    done
